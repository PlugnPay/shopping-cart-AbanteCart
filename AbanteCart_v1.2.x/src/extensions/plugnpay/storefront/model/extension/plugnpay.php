...
class ModelExtensionPlugnpay extends Model
{
    public function getMethod($address)
    {
        $this->load->language('plugnpay/plugnpay');

        if ($this->config->get('plugnpay_status')) {
            $query = $this->db->query("SELECT *
										FROM ".$this->db->table("zones_to_locations")."
										WHERE location_id = '".(int)$this->config->get('plugnpay_location_id')."'
												AND country_id = '".(int)$address['country_id']."'
												AND (zone_id = '".(int)$address['zone_id']."' OR zone_id = '0')");

            if (!$this->config->get('plugnpay_location_id')) {
                $status = true;
        ...
        if ($status) {
            $method_data = array(
                ...
                'sort_order' => $this->config->get('plugnpay_sort_order'),
            );
        }
