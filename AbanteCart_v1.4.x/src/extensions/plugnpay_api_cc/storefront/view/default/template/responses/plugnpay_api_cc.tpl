...
<div class="form-group form-inline">
	...
	<div class="col-sm-7 input-group"><?php echo $cc_number; ?></div>
	<span class="help-block"></span>
</div>
<div class="form-group form-inline">
	...
	jQuery(document).ready(function () {
		...
		function confirmSubmit($form) {
			$.ajax({
				...
				beforeSend: function () {
					$('.alert').remove();
					$('#plugnpay .action-buttons').hide();
					$('#plugnpay .action-buttons').before('<div class="wait alert alert-info text-center"><i class="fa fa-refresh fa-spin"></i> <?php echo $text_wait; ?></div>');
				},
				success: function (data) {
					if (!data) {
						$('.wait').remove();
						$('#plugnpay .action-buttons').show();
						$('#plugnpay').before('<div class="alert alert-danger"><i class="fa fa-bug"></i> <?php echo $error_unknown; ?></div>');
						submitSent = false;
						try { resetLockBtn(); } catch (e) { }
					} else {
						if (data.error) {
							$('.wait').remove();
							$('#plugnpay .action-buttons').show();
							$('#plugnpay').before('<div class="alert alert-warning"><i class="fa fa-exclamation"></i> ' + data.error + '</div>');
							submitSent = false;
				...
				error: function (jqXHR, textStatus, errorThrown) {
					$('.wait').remove();
					$('#plugnpay .action-buttons').show();
					$('#plugnpay').before('<div class="alert alert-danger"><i class="fa fa-exclamation"></i> ' + textStatus + ' ' + errorThrown + '</div>');
					submitSent = false;
					try {	resetLockBtn();	} catch (e) {}
				}
			});
